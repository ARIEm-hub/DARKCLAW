...
// boot sequence
(function initBoot(){
  const fast = sessionStorage.getItem('darkclaw_v10_origin_boot') === 'done';
  if(fast) body.classList.add('quick-boot');
  const status = $('#boot-status');
  const step = $('#boot-step');
  const cometScene = $('.boot-comet-scene');
  const cometPath = $('#comet-path');
  const cometDot = $('#comet-dot');
  const trailBase = $('.trail-base');
  const trailGlow = $('.trail-glow');
  const isMobile = window.innerWidth <= 760;
  let pathLength = 0;
  try { pathLength = cometPath && typeof cometPath.getTotalLength === 'function' ? cometPath.getTotalLength() : 0; } catch (e) { pathLength = 0; }
  if(pathLength){
    [trailBase, trailGlow].forEach(path => {
      if(!path) return;
  ...
  const phases = ['boot-eyes','boot-comet','boot-rings','boot-bloom','boot-title','boot-final','boot-open'];
  function setPhase(className, phaseStep, phaseStatus){
    phases.forEach(c => body.classList.remove(c));
    if(className) body.classList.add(className);
    if(step) step.textContent = phaseStep;
  ...
  function resetTrail(){
    ...
    if(cometDot) cometDot.style.transform = 'translate(-9999px,-9999px)';
  }

  function runComet(duration = 1000){
    if(!cometPath || !pathLength || !cometDot) return;
    resetTrail();
    cometScene?.classList.add('is-running');
    const start = performance.now();
    function frame(now){
      const t = Math.min((now - start) / duration, 1);
      const eased = 1 - Math.pow(1 - t, 3);
      const draw = pathLength * eased;
      const pos = cometPath.getPointAtLength(draw);
      cometDot.style.transform = `translate(${pos.x}px, ${pos.y}px)`;
      [trailBase, trailGlow].forEach(path => {
        if(!path) return;
        path.style.strokeDasharray = `${draw} ${pathLength}`;
        path.style.strokeDashoffset = `${Math.max(pathLength - draw, 0)}`;
      });
  ...
  const schedule = isMobile ? [
    [120, ()=> setPhase('boot-eyes','01 / 07','ПОГРУЖЕНИЕ ВО ТЬМУ')],
    [1050, ()=> { setPhase('boot-comet','02 / 07','СВЕТОВОЙ СЛЕД ПРОБУЖДЁН'); runComet(fast ? 520 : 860); }],
    [2080, ()=> setPhase('boot-rings','03 / 07','ЖИДКОЕ СТЕКЛО РАСПРОСТРАНЯЕТСЯ')],
    [2920, ()=> setPhase('boot-bloom','04 / 07','ФОРМИРУЮТСЯ СТЕКЛЯННЫЕ ЛЕПЕСТКИ')],
    [4020, ()=> setPhase('boot-title','05 / 07','У КАЖДОЙ СТАИ ЕСТЬ НАЧАЛО')],
    [5000, ()=> { setPhase('boot-final','06 / 07','И ЭТО — НАШЕ'); runComet(fast ? 420 : 620); }],
    [5920, ()=> setPhase('boot-open','07 / 07','DARKCLAW ОТКРЫВАЕТСЯ')]
  ] : [
    [120, ()=> setPhase('boot-eyes','01 / 07','ПОГРУЖЕНИЕ ВО ТЬМУ')],
    [1180, ()=> { setPhase('boot-comet','02 / 07','СВЕТОВОЙ СЛЕД ПРОБУЖДЁН'); runComet(fast ? 640 : 980); }],
    [2320, ()=> setPhase('boot-rings','03 / 07','ЖИДКОЕ СТЕКЛО РАСПРОСТРАНЯЕТСЯ')],
    [3220, ()=> setPhase('boot-bloom','04 / 07','ФОРМИРУЮТСЯ СТЕКЛЯННЫЕ ЛЕПЕСТКИ')],
    [4520, ()=> setPhase('boot-title','05 / 07','У КАЖДОЙ СТАИ ЕСТЬ НАЧАЛО')],
    [5600, ()=> { setPhase('boot-final','06 / 07','И ЭТО — НАШЕ'); runComet(fast ? 520 : 760); }],
    [6580, ()=> setPhase('boot-open','07 / 07','DARKCLAW ОТКРЫВАЕТСЯ')]
  ];
  const scale = fast ? .55 : 1;
  schedule.forEach(([ms, fn]) => setTimeout(fn, ms * scale));
  setTimeout(() => {
    body.classList.remove(...phases, 'is-booting');
    if(boot) boot.remove();
    sessionStorage.setItem('darkclaw_v10_origin_boot','done');
    setTimeout(showAriConsent, 260);
  }, isMobile ? (fast ? 3600 : 6900) : (fast ? 3900 : 7550));
})();
...
document.addEventListener('keydown', e => { if((e.ctrlKey || e.metaKey) && ['s','c'].includes(e.key.toLowerCase()) && document.activeElement.tagName !== 'INPUT'){ e.preventDefault(); } });
